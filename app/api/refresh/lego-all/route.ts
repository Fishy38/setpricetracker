...
async function refreshOne(input: RefreshInput) {
  const { setId, legoUrl } = input;

  const fetchUrl = normalizeLegoUrl(legoUrl);

  if (!fetchUrl) {
    return { setId, ok: false, error: "Missing/invalid legoUrl" as const, legoUrl };
  }
  ...
  if (!res.ok) {
    return { setId, ok: false, error: `LEGO fetch failed: ${res.status}` as const, legoUrl: fetchUrl };
  }
  ...
  // Normalize Set.legoUrl in DB (so next time it's already canonical)
  try {
    if (legoUrl !== fetchUrl) {
